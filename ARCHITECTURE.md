...
**Version 0.50 | Event-Driven Modular Architecture**
...
- **Storage:** `localStorage` primary + `IndexedDB` fallback for save data (Android resilience); JSON import/export for portability
...
| `save.js` | localStorage + IndexedDB save/load, import/export | 1 |
| `cadence.js` | Pedalling input, CPM measurement, click bridge | 2 |
| `bluetooth.js` | BLE sensor connection, FTMS/CSC/iConsole parsers | 2 |
| `BikeConfig.js` | Bike channel mapping UI, instrument panel | 2 |
...
| `worldmap.js` | World travel UI, procedural map, road encounters | 3 |
...
State.data = {
  ...
  cadence: {
    ...
    targetCPM: 90,         // Normal activity target (changed from 60 in v0.47)
    raidTargetCPM: 110,    // Target during raid combat (changed from 90 in v0.47)
...
→ cadence.js → bluetooth.js → BikeConfig.js → player.js → daynight.js → animals.js
→ SceneryCanvas.js → GroundCanvas.js → Lights.js → BaseLights.js
→ BuildPrompt.js → InlineBuildings.js → dynamo_bike.js
→ buildings/*.js → base.js → map.js → worldmap.js → foraging.js
→ crafting.js → power.js → achievements.js → farming.js → playerstats.js
→ settings.js → devmode.js → main.js
```
...
## 9.7 Bluetooth System — BLE Sensor Input

> **Added in v0.46.** Replaces direct cadence registration with a full BLE abstraction layer.

### Overview

`bluetooth.js` manages all BLE sensor connections. It is a Layer 2 module that writes CPM into `State.data.cadence.clicksPerMinute` and calls `Cadence.registerClick()` to synthesise timestamps. No other module touches BLE.

`BikeConfig.js` is the channel mapping UI — it opens automatically 800ms after `bike:connected` fires if `hasMapping` is false.

### Protocol Detection Order

Detection always proceeds: **FTMS → CSC → iConsole**. iConsole is skipped if the notify characteristic has no properties (Chrome Web Bluetooth bug on some Android devices).

| Protocol | BLE Service | Characteristic | Notes |
|----------|-------------|----------------|-------|
| FTMS | `1826` | Data: `2ad2`, Control Point: `2ad9` | Used by iConsole+0060 and most smart trainers |
| CSC | `1816` | Measurement: `2a5b` | Cadence-only sensors (Magene S3 etc.) |
| iConsole | `fff0` | `fff1` / `fff4` | Legacy iConsole protocol, skipped if no notify properties |

### FTMS Subscription Sequence

```js
// 1. Subscribe to CP indications (2ad9)
// 2. writeValue([0x00])  → Request Control
// 3. await 200ms
// 4. writeValue([0x07])  → Start or Resume
```

### Cadence Bridge

The CSC parser and iConsole parser produce raw revolution counts with timestamps. `bluetooth.js` synthesises individual `Cadence.registerClick()` calls by distributing `deltaRevs` clicks across `deltaTime` using evenly-spaced synthetic timestamps. This makes the CPM algorithm in `cadence.js` work identically for both button-tapped and sensor-derived cadence.

### Key Events

| Event | Payload | Who emits | Who handles |
|-------|---------|-----------|-------------|
| `bike:connected` | `{ name, protocol }` | `bluetooth.js` | `BikeConfig.js` (auto-open if no mapping), `main.js` HUD |
| `bike:disconnected` | — | `bluetooth.js` | `main.js` HUD |
| `bike:cpm` | `{ cpm }` | `bluetooth.js` | `cadence.js` bridge |

### CPM Fallback Rule

**All CPM fallbacks must use `|| 90`, never `|| 60`.** The targets changed in v0.47:
- `State.data.cadence.targetCPM = 90` (normal speed)
- `State.data.cadence.raidTargetCPM = 110` (fast/raid speed)
- 60 CPM = slow/dangerous threshold, not the target

Any new code reading `targetCPM` with a fallback must write `|| 90`.

---

## 9.8 World Travel System

> **Redesigned in v0.47–v0.48.**

### Travel Time Formula

```js
const maxDist    = 2500;                          // world units, farthest zone
const normDist   = Utils.clamp(dist / maxDist, 0, 1);
const normalTime = Math.max(30, Math.round(normDist * 1200)); // seconds, max 20min
const slowTime   = Math.round(normalTime * 1.5);
const fastTime   = Math.round(normalTime * 0.6);
const baseSpeed  = 1 / (normalTime * 1.15);      // ratio=1.0 arrives exactly on time
const speed      = baseSpeed * (0.3 + ratio * 0.85);
```

`ratio = cpm / targetCPM`. At `ratio = 1.0`, the player arrives in exactly `normalTime` seconds.

### GO Button Rule

The GO button **must use `onclick=` assignment**, not `addEventListener`. This prevents listener stacking when the player taps multiple map locations before travelling — stacking caused the first (smallest) `normalTime` to fire immediately on travel start.

```js
// CORRECT
goBtn.onclick = () => this._startTravel(locationId, ...);

// WRONG — stacks a new listener every time a location is tapped
goBtn.addEventListener('click', () => this._startTravel(...));
```

### Travel HUD — ETA Display

`_updateTravelHUD(progress, cpm, ratio, speed, normalTime)` now shows estimated time remaining alongside percentage. ETA is computed as `(1 - progress) / speed` seconds, formatted as `Xm Ys left`. The ETA colour turns green when current speed exceeds normal pace.

The `speed` and `normalTime` parameters were added in v0.50 — always pass them when calling this method.

---

## 9.9 Combat System — CPM-Driven Auto-Attack

> **Redesigned in v0.50.** Replaces click-gated damage with a cadence accumulator.

### Design Principle

Both road encounters (`worldmap.js`) and foraging encounters (`foraging.js`) use an **attack accumulator** (`_atkAccum`) that fills at a rate proportional to CPM. This means cadence sensor users deal full damage without ever pressing a button. Button/tap presses add a bonus hit on top.

### Attack Rate Formula

```js
// attacks per second = ratio * 1.5
// where ratio = cpm / targetCPM

//  60 CPM (ratio ≈ 0.67) → ~1.0 attacks/sec
//  90 CPM (ratio = 1.0)  → ~1.5 attacks/sec
// 110 CPM (ratio ≈ 1.22) → ~1.8 attacks/sec
```

### Accumulator Pattern (both worldmap.js and foraging.js)

```js
// On encounter start — always reset:
this._atkAccum = 0;
...
if (hits > 0) {
  ...
  // show hit feedback with damage numbers
}
```
...
```js
// In _roadEncounterClick() / _clickDamage():
const dmg = Utils.randFloat(3, 7) * Math.max(0.5, ratio);
this._encounterHP -= dmg;
this._atkAccum += 0.4;  // boost accumulator — next auto-attack fires sooner
```

### Lose Timer Behaviour

| CPM | Road encounter | Foraging encounter |
|-----|---------------|-------------------|
| < ~36 (ratio < 0.4) | Lose timer++ per tick | Lose timer++ per tick |
| 36–63 (ratio 0.4–0.7) | Timer holds (slow charge) | Timer holds |
| 63–90 (ratio 0.7–1.0) | Attacks fire ~1/sec, timer retreats | Attacks fire ~1/sec |
| 90+ (ratio 1.0+) | Attacks fire 1.5+/sec, timer retreats fast | Same |

The `_lastEncounterClick` timestamp from the old system has been **removed**. Do not re-introduce click-gating.

---

## 9.10 Save System — Android IndexedDB Fallback

> **Added in v0.49.**

`save.js` now tries `localStorage` first, then falls back to `IndexedDB` silently. All three public save methods are now **async**:

```js
// CORRECT — always await these
await SaveSystem.saveLocal();
const data = await SaveSystem.loadLocal();
const exists = await SaveSystem.hasSave();
```

Internal IDB helpers: `_idbOpen()`, `_idbWrite(key, value)`, `_idbRead(key)`. Database name: `'PedalOrDieDB'`, store name: `'saves'`.

**Any caller that uses these methods must `await` them.** Forgetting `await` on `hasSave()` or `loadLocal()` will cause silent failures — the promise object is truthy even when the underlying value is absent.

---

## 9.11 HUD Fold-Out Menu

> **Added in v0.49.**

The six individual HUD action buttons are collapsed into a single `⋮` button that expands vertically on tap. The `HudMenu` global object lives in `main.js`.
...
A capture-phase `pointerdown` listener on `document` closes the menu on any outside tap. The menu closes automatically when any item inside it is activated.

Do not add new standalone HUD buttons. If a new action is needed, add it as an item inside the `HudMenu` expansion list.

---

## 9.12 Resource Help Tooltips

> **Added in v0.49.**

When a building's upgrade screen shows a resource the player lacks, that resource label is tappable. `crafting._showResourceHelp(resourceId)` renders a panel explaining where to find or craft the resource.

The lookup map is `_locationHints` inside `crafting.js`. When adding a new resource to the game, add a corresponding entry to `_locationHints`:

```js
_locationHints: {
  my_resource: 'Found by foraging in the Ruins zone, or crafted at the Workshop.',
  // ...
}
```
...
## 9.13 Dynamo Bike — Always-On Mode

> **Added in v0.47.**

`State.data.power.dynamoActive` (boolean, default `false`) controls whether the dynamo bike generates power passively while the player is on the base or map screen, without opening the dynamo bike screen.

When `dynamoActive` is true:
- A 1-second tick (wired at `game:boot`) checks `State.data.cadence.clicksPerMinute >= 10`
- If pedalling: calls `Power.addEnergy(watts)` proportional to CPM
- The toggle is exposed in the dynamo bike building screen

Do not generate passive power from any other path. All dynamo power generation goes through `Power.addEnergy()`.

---

## 9.14 Electric Fence — Consumable Charges

> **Added in v0.49.**

The electric fence (building key `fence`) auto-unlocks and enables the `elecFence` power consumer at level 9. Each raid consumes one `elec_fence_upgrade` item from inventory. The fence only activates if:

```js
State.data.power.consumers.elecFence === true  // toggle on
&& Power.hasPower()                             // battery has charge
&& State.data.inventory.elec_fence_upgrade > 0  // has charges
```

Craft 10 → fence works for 10 raids. The item is deducted in `raids._onFenceCheck()`, not in `power.js`.

---

## 9.15 Construction Speed Bonus

> **Added in v0.47.**

The build timer bonus per tick scales with current CPM ratio:

```js
// In crafting._tickBuild():
const bonus = ratio * 2.0;   // was hardcoded 0.5 before v0.47
ab.secsLeft = Math.max(0, ab.secsLeft - 1 - bonus);
```

At `ratio = 1.0` (90 CPM): 3× real-time build speed. At `ratio = 1.22` (110 CPM): ~4.5× speed. At zero CPM: 1× (no bonus).
...
*This document reflects the codebase at version 0.50. It covers the full event-driven refactor (v0.17), the power system redesign (v0.17), the BLE bluetooth abstraction layer (v0.46), CPM target recalibration (v0.47), travel time realism and combat threshold fixes (v0.47–v0.48), and the v0.49–v0.50 additions: electric fence consumable charges, woodburner/coal plant UI, Android IndexedDB save fallback, HUD fold-out menu, resource help tooltips, Android tap double-fire fix, CPM-driven auto-attack combat, and travel ETA display. The architecture has reduced inter-module coupling from 100+ direct cross-references to a controlled set of ~22 documented functional dependencies, with all other communication flowing through the Event Bus.*
