...
const Power = {
  ...
  // ── Max battery storage ───────────────────
  getMaxStorage(lvl) {
    const level = lvl !== undefined ? lvl : (State.data?.power?.battery?.level || 0);
    return this._batteryCapacity[Math.min(level, 10)] || 0;
  },
...
const BuildingBatteryBankScreen = {
  getScreenData(s) {
    ...
    const phLv    = s.base.buildings.powerhouse?.level || 0;
    ...
    // Locked: Power House not built yet
    if (phLv < 1) {
      const statsRows = `
        <div class="bsc-row locked"><span>Status</span><span>🔒 Locked</span></div>
        <div class="bsc-row"><span>Unlock</span><span>Build the Power House first</span></div>
        <div class="bsc-row"><span>Purpose</span><span>Stores electricity for use at night and during raids</span></div>`;
      return { title: '🔋 BATTERY BANK', visual, statsRows, actionBtn: '', skipUpgradeSection: true };
    }

    // Not yet built
    if (lv === 0) {
      const parts = (typeof Power !== 'undefined') ? Power._batteryParts(1) : {};
      const hasAll = (typeof Power !== 'undefined') ? Power._hasAllBatteryParts(1) : false;
      const partsHtml = Object.entries(parts).map(([res, amt]) => {
        const have = s.inventory[res] || 0;
        const ok   = have >= amt;
        return `<div class="bsc-row ${ok?'ok':'bad'}"><span>${Utils.emojiMap?.[res]||'📦'} ${res.replace(/_/g,' ')}</span><span>${have}/${amt}</span></div>`;
      }).join('');
      const statsRows = `
        <div class="bsc-row locked"><span>Status</span><span>🔒 Not yet built</span></div>
        <div class="bsc-row"><span>Purpose</span><span>Stores power generated by dynamo, solar and woodburner</span></div>
        <div class="bsc-row"><span colspan="2" style="color:#aaa;font-size:0.85em">Parts needed to build Lv1:</span></div>
        ${partsHtml}`;
      const actionBtn = `<button class="btn-pixel btn-primary" style="width:100%;margin-top:8px"
        onclick="Power.buildBattery()" ${hasAll?'':'disabled'}>
        🔋 BUILD BATTERY BANK${hasAll?'':' — Missing parts'}
      </button>
      <div style="color:#888;font-size:0.8em;margin-top:4px;text-align:center">
        Craft battery cells at the Electric Bench
      </div>`;
      return { title: '🔋 BATTERY BANK', visual, statsRows, actionBtn, skipUpgradeSection: true };
    }

    // Built — show status and upgrade button
    const nextLv = lv + 1;
    const nextMax = (typeof Power !== 'undefined') ? Power.getMaxStorage(nextLv) : 0;
    const parts   = (typeof Power !== 'undefined') && lv < 10 ? Power._batteryParts(nextLv) : {};
    const hasAll  = (typeof Power !== 'undefined') && lv < 10 ? Power._hasAllBatteryParts(nextLv) : false;
    const partsHtml = lv < 10 ? Object.entries(parts).map(([res, amt]) => {
      const have = s.inventory[res] || 0;
      const ok   = have >= amt;
      return `<div class="bsc-row ${ok?'ok':'bad'}"><span>${Utils.emojiMap?.[res]||'📦'} ${res.replace(/_/g,' ')}</span><span>${have}/${amt}</span></div>`;
    }).join('') : '';
    ...
    const statsRows = `
      ...
      ${lv < 10 ? `<div class="bsc-row" style="margin-top:8px"><span colspan="2" style="color:#aaa;font-size:0.85em">Parts for Lv${nextLv} (${nextMax} Wh):</span></div>${partsHtml}` : ''}`;

    const actionBtn = lv >= 10
      ? `<div class="bsc-upgrade-status maxed" style="margin-top:8px">✅ MAXIMUM LEVEL REACHED</div>`
      : `<button class="btn-pixel btn-primary" style="width:100%;margin-top:8px"
           onclick="Power.buildBattery()" ${hasAll?'':'disabled'}>
           🔋 UPGRADE TO LV${nextLv}${hasAll?'':' — Missing parts'}
         </button>`;
